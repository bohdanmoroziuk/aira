...
- Server: API handlers (`server/api/**`) stay thin (parse, call a use-case, shape the response) — no business logic, no direct persistence/SDK calls. Use-cases depend only on port types. Project use-cases live in flows (`server/flows/<kebab-name>/`: `<name>.use-case.ts`, `<name>.use-case.test.ts`, optional `<name>.schema.ts`, and an `index.ts` re-exporting the schema and use case); chat use-cases are still in `server/use-cases/` until they migrate. Flows keep their own schema and test beside the use case; schemas shared by several flows stay in `server/schemas/`. Repositories/services (`server/repositories/`, `server/services/`) are adapters only. Composition roots (`*.container.ts`) only wire, no logic.
